#!/usr/bin/env python3
"""
Cloudinary responsive <picture> generator with JXL / AVIF / WebP / JPEG output.
...
def generate_picture_html(
    *,
    ...
    original_width = int(cloudinary_data.get("width") or max(widths_by_format[source_formats[0]]))
    ...
    fallback_widths = widths_by_format.get(fallback_fmt) or widths_by_format.get("webp") or widths_by_format[source_formats[-1]]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload image to Cloudinary and generate JXL/AVIF/WebP responsive <picture> HTML with a JPEG <img> fallback.")
    ...
    parser.add_argument("--fallback-format", default=DEFAULT_FALLBACK_FORMAT, help="Fallback <img> format. Default: jpeg")
    ...
    source_formats = [normalize_format(fmt) for fmt in args.formats]
    fallback_format = normalize_format(args.fallback_format)
    generated_formats = list(dict.fromkeys([*source_formats, fallback_format]))
    ...
    try:
        ...
        widths_by_format = decouple_widths_by_format(selected_widths, generated_formats, enabled=not args.no_decouple)
